...
from lms.djangoapps.discussion.django_comment_client.utils import is_commentable_divided
...
class ViewsTestCase(
    ...
    def test_create_cohorted_thread(self, mock_request):
        ...
        assert is_commentable_divided(self.course.id, commentable_id)
        ...
        assert get_notifications_count_for_user(self.student.id) == 0
        ...
        with patch('social_engagement.engagement._get_user_social_stats') as mock_func:
            ...
            assert get_notifications_count_for_user(self.student.id) == 1
        ...
        assert get_notifications_count_for_user(a_user.id) == 1

        # people not in the cohort, should not get the notification
        assert get_notifications_count_for_user(b_user.id) == 0
        assert get_notifications_count_for_user(no_user.id) == 0
...
class ForumThreadViewedEventTransformerTestCase(ForumsEnableMixin, UrlResetMixin, ModuleStoreTestCase):
    ...
    def test_categories(self):
        ...
        full_category_name = '{} / {}'.format(self.category.discussion_category, self.category.discussion_target)
